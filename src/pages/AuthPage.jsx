...
export default function AuthPage({onLogin,onSignup,onForgotPassword,otpPending,otpSending,onVerifyOTP,onCancelOTP,onResendOTP}){
  ...
  const handleResend=async()=>{
    ...
    setOtpErr('');
    const result=await onResendOTP(otpPending.email,otpPending.isAdmin);
          ...
          <div style={{background:'rgba(255,255,255,0.92)',borderRadius:24,padding:'32px 28px',boxShadow:'0 25px 80px rgba(0,0,0,0.4)',backdropFilter:'blur(24px)',border:'1px solid rgba(255,255,255,0.2)'}}>
            ...
            {otpPending?<>
              <div style={{textAlign:'center',marginBottom:20}}>
                ...
                <p style={{fontSize:13,color:'#64748B',margin:0}}>Tumetuma code kwenye {otpPending.isAdmin?'SMS':'email yako'}</p>
                <p style={{fontSize:15,fontWeight:700,color:'#0B7A3B',margin:'6px 0 0'}}>{otpPending.isAdmin?'📱 ***6770':otpPending.email?.replace(/(.{2})(.*)(@.*)/,'$1***$3')}</p>
              </div>
