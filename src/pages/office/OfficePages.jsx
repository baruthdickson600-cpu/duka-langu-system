...
export function OfficeDash({onReceipt}){
  ...
  const BIZ_TYPE_CONFIG={
    retail:    {icon:'🏪',label:'Retail Shop',   color:'#3B82F6'},
    fashion:   {icon:'👗',label:'Fashion',        color:'#EC4899'},
    hardware:  {icon:'🔩',label:'Hardware',       color:'#F59E0B'},
    agrovet:   {icon:'🌱',label:'Agrovet',        color:'#22C55E'},
    restaurant:{icon:'🍽',label:'Restaurant',     color:'#EF4444'},
    pharmacy:  {icon:'💊',label:'Pharmacy',       color:'#8B5CF6'},
    electronics:{icon:'💻',label:'Electronics',   color:'#06B6D4'},
    stationery:{icon:'📚',label:'Stationery',     color:'#F97316'},
    beauty:    {icon:'💇',label:'Beauty Salon',   color:'#EC4899'},
    classic:   {icon:'⭐',label:'Classic Mode',   color:'#64748B'},
    other:     {icon:'📦',label:'Biashara',       color:'#64748B'},
  };
  ...
  return <div>
    ...
    <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16,flexWrap:'wrap'}}>
      <div style={{display:'inline-flex',alignItems:'center',gap:8,background:`${bizTypeCfg.color}15`,border:`1.5px solid ${bizTypeCfg.color}40`,borderRadius:20,padding:'6px 14px'}}>
        <span style={{fontSize:18}}>{bizTypeCfg.icon}</span>
        <div>
          <span style={{fontSize:12,fontWeight:700,color:bizTypeCfg.color}}>{bizTypeCfg.label}</span>
          {biz?.name&&<span style={{fontSize:11,color:'#64748B',marginLeft:6}}>• {biz.name}</span>}
        </div>
      </div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:16}}>
      ...
      {/* AI SMART INSIGHTS — Basic na zaidi */}
...
export function SalesPage({onDone}){
  ...
  const addToCart=(p,useWholesale=false)=>{
    ...
    unlockAudio(); // Ruhusu sauti (browser autoplay policy)
  ...
  if(empNoBranch)return <div style={{padding:40,textAlign:'center',background:'#FFF7ED',borderRadius:14,border:'1.5px solid #FED7AA',maxWidth:500,margin:'40px auto'}}>
    ...
    <div style={{fontSize:16,fontWeight:800,color:'#9A3412',marginBottom:8}}>Hujapangiwa Tawi</div>
    <div style={{fontSize:13,color:'#92400E',lineHeight:1.6}}>Mmiliki wa biashara lazima akupangie tawi kabla ya kuanza kuuza. Wasiliana naye akupangie tawi lako.</div>
  </div>;
  ...
  return <div>
    ...
    <div style={{display:'flex',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:8}}>
      ...
      <div style={{display:'flex',gap:8}}>
        {canUseBranches&&myBranches.length>=2&&<Btn v="ghost" onClick={()=>setCopyModal(true)} style={{background:'#EFF6FF',color:'#2563EB'}}>📋 Nakili kwa Tawi</Btn>}
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:12}}>
      {filtered.map(p=>{const isExp=p.expiry_date&&new Date(p.expiry_date)<new Date();const margin=p.buy_price&&p.sell_price?((p.sell_price-p.buy_price)/p.sell_price*100):100;
        ...
        {canUseBranches&&p.branch_id&&(()=>{const br=myBranches.find(b=>b.id===p.branch_id);return br?<div style={{fontSize:10,color:'#0B7A3B',marginTop:2,fontWeight:600}}>🏪 {br.name}</div>:null;})()}
        {canUseBranches&&!p.branch_id&&<div style={{fontSize:10,color:'#94A3B8',marginTop:2}}>📦 Bidhaa ya jumla</div>}
    ...
    {/* Nakili Bidhaa kwa Tawi Lingine */}
    <Modal open={copyModal} onClose={()=>{setCopyModal(false);setCopyTo('')}} title="📋 Nakili Bidhaa kwa Tawi">
      <div style={{fontSize:13,color:'#64748B',marginBottom:14,lineHeight:1.5}}>
        Bidhaa za {activeBranch?(myBranches.find(b=>b.id===activeBranch)?.name||'tawi hili'):'jumla'} zitanakiliwa kwa tawi utakalochagua. Stock itaanza 0 — utajaza baadaye.
      </div>
      <Sel label="Nakili kwenda tawi:" value={copyTo} onChange={e=>setCopyTo(e.target.value)} options={[
        {value:'',label:'-- Chagua Tawi --'},
        ...myBranches.filter(b=>b.id!==activeBranch).map(b=>({value:b.id,label:b.name})),
      ]}/>
      <Btn disabled={!copyTo||copying} onClick={async()=>{
        setCopying(true);
        const r=await copyProductsToBranch(activeBranch||null,copyTo);
        setCopying(false);
        if(r.success){alert(`✅ Bidhaa ${r.copied} zimenakiliwa! Nenda tawi hilo ujaze stock.`);setCopyModal(false);setCopyTo('');}
        else{alert('⚠️ '+r.error);}
      }} style={{width:'100%',justifyContent:'center',marginTop:8}}>{copying?'Inanakili...':'Nakili Sasa'}</Btn>
    </Modal>
  </div>;
}
...
export function ReportsPage({onReceipt}){
  const{sales,returns,products,expenses,currency,creditHistory=[],allSales,allExpenses,branches,canUseBranches,getBranches,activeBranch}=useApp();
  const fm=n=>fmtMoney(n,currency||'TZS');
  ...
  return <div>
    ...
    <div style={{marginBottom:14}}>
      ...
      <p style={{fontSize:12,color:'#64748B',margin:0}}>
        {canUseBranches&&activeBranch?`Tawi: ${myBranches.find(b=>b.id===activeBranch)?.name||''}`:canUseBranches?'Matawi Yote (Pamoja)':'Angalia mauzo ya leo, wiki, mwezi, au chagua tarehe maalum'}
      </p>
    </div>

    {/* MCHANGANUO KWA TAWI - tu kwenye Matawi Yote */}
    {canUseBranches&&!activeBranch&&myBranches.length>0&&(()=>{
      // Hesabu takwimu kwa kila tawi (mauzo yote ya leo)
      const todayStr2=new Date().toISOString().slice(0,10);
      const branchStats=myBranches.map(br=>{
        const bSales=(allSales||[]).filter(s=>s.branch_id===br.id&&s.created_at?.startsWith(todayStr2));
        const bExp=(allExpenses||[]).filter(e=>e.branch_id===br.id&&e.created_at?.startsWith(todayStr2));
        return{
          branch:br,
          sales:bSales.reduce((a,s)=>a+(s.total||0),0),
          profit:bSales.reduce((a,s)=>a+(s.profit||0),0),
          expenses:bExp.reduce((a,e)=>a+(e.amount||0),0),
          count:bSales.length,
        };
      });
      const grandTotal=branchStats.reduce((a,b)=>a+b.sales,0);
      return <div style={{background:'#fff',borderRadius:14,padding:16,marginBottom:16,boxShadow:'0 1px 4px rgba(0,0,0,.06)',border:'1px solid #E2E8F0'}}>
        <div style={{fontWeight:800,fontSize:15,marginBottom:12,display:'flex',alignItems:'center',gap:8}}>
          🏪 Mchanganuo kwa Tawi (Leo)
        </div>
        <div style={{overflowX:'auto'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
            <thead>
              <tr style={{borderBottom:'2px solid #E2E8F0',textAlign:'left'}}>
                <th style={{padding:'8px 6px',color:'#64748B',fontSize:11}}>TAWI</th>
                <th style={{padding:'8px 6px',color:'#64748B',fontSize:11,textAlign:'right'}}>MAUZO</th>
                <th style={{padding:'8px 6px',color:'#64748B',fontSize:11,textAlign:'right'}}>FAIDA</th>
                <th style={{padding:'8px 6px',color:'#64748B',fontSize:11,textAlign:'right'}}>MATUMIZI</th>
                <th style={{padding:'8px 6px',color:'#64748B',fontSize:11,textAlign:'right'}}>RISITI</th>
              </tr>
            </thead>
            <tbody>
              {branchStats.map(st=>(
                <tr key={st.branch.id} style={{borderBottom:'1px solid #F1F5F9'}}>
                  <td style={{padding:'10px 6px',fontWeight:600}}>🏪 {st.branch.name}</td>
                  <td style={{padding:'10px 6px',textAlign:'right',fontWeight:700,color:'#0B7A3B'}}>{fm(st.sales)}</td>
                  <td style={{padding:'10px 6px',textAlign:'right',color:'#15803D'}}>{fm(st.profit)}</td>
                  <td style={{padding:'10px 6px',textAlign:'right',color:'#EF4444'}}>{fm(st.expenses)}</td>
                  <td style={{padding:'10px 6px',textAlign:'right',color:'#64748B'}}>{st.count}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr style={{borderTop:'2px solid #E2E8F0',fontWeight:800}}>
                <td style={{padding:'10px 6px'}}>JUMLA</td>
                <td style={{padding:'10px 6px',textAlign:'right',color:'#0B7A3B'}}>{fm(grandTotal)}</td>
                <td style={{padding:'10px 6px',textAlign:'right',color:'#15803D'}}>{fm(branchStats.reduce((a,b)=>a+b.profit,0))}</td>
                <td style={{padding:'10px 6px',textAlign:'right',color:'#EF4444'}}>{fm(branchStats.reduce((a,b)=>a+b.expenses,0))}</td>
                <td style={{padding:'10px 6px',textAlign:'right'}}>{branchStats.reduce((a,b)=>a+b.count,0)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
        <div style={{fontSize:11,color:'#94A3B8',marginTop:8}}>💡 Chagua tawi kwenye menu ya pembeni kuona ripoti kamili ya tawi hilo</div>
      </div>;
    })()}
...
export function BranchesPage(){
  ...
  return <div>
    ...
    <Modal open={modal} onClose={()=>setModal(false)} title="\ud83c\udfea Tawi Jipya">
      ...
      <Btn onClick={async()=>{
        if(!f.name.trim())return alert('Weka jina la tawi!');
        const location=[f.region,f.district].filter(Boolean).join(', ');
        await addBranch(f.name.trim(),location,{address:f.address,phone:f.phone,is_active:f.is_active!==false});
        setModal(false);
    ...
    <Modal open={editModal.open} onClose={()=>setEditModal({open:false,branch:null})} title="\u270f\ufe0f Hariri Tawi">
      {editModal.branch&&<>
        ...
        <Input label="Wilaya" value={editModal.branch.district||''} onChange={e=>setEditModal({...editModal,branch:{...editModal.branch,district:e.target.value}})}/>
        <Input label="Anwani" value={editModal.branch.address||''} onChange={e=>setEditModal({...editModal,branch:{...editModal.branch,address:e.target.value}})}/>
        <Input label="Simu" value={editModal.branch.phone||''} onChange={e=>setEditModal({...editModal,branch:{...editModal.branch,phone:e.target.value}})}/>
        <div style={{marginBottom:14}}>
          <label style={{fontSize:12,fontWeight:600,color:'#64748B',display:'block',marginBottom:4}}>Hali</label>
